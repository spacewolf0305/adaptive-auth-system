...
\begin{abstract}
Traditional authentication relies on static, binary accept/reject decisions, leaving systems vulnerable to credential theft. This paper presents an Adaptive Authentication System that leverages a Random Forest classifier trained on contextual features to dynamically assess risk. Instead of a binary decision, the model drives a configurable three-tier proportional-response framework: ALLOW, MFA (SMS OTP), and BLOCK. Evaluated on a 10,000-event synthetic benchmark, the model achieves 94.45\% accuracy (ROC-AUC 0.9019) and 43ms median inference latency. Security testing against 1,500 adversarial scenarios yields 100\% detection of high-risk attacks. However, a novel adaptive-adversary evasion-cost analysis reveals that 100\% of blocked attackers can evade detection by altering just 1.03 features on average---primarily the IP Threat Score. We conclude that while contextual risk scoring provides strong baseline security with zero false positives, true adversarial robustness requires the integration of harder-to-spoof behavioral biometrics. All code and a reference AWS deployment are open-sourced.
\end{abstract}
...
\begin{cases}
\text{ALLOW}, & r < 0.3 \\
\text{MFA (e.g., SMS OTP challenge)}, & 0.3 \le r < 0.7 \\
...
The contextual features are intended to raise the cost of these capabilities to differing degrees. Location and distance features (Country, Region, Distance) resist C2 by requiring the attacker to additionally co-locate with, or accurately mimic, the victim's usual geography. Threat Score (based on Network Profiles such as Tor Exit Nodes, VPNs, or Residential IPs) resists C2 when the attacker reuses flagged infrastructure but degrades against clean residential proxies. Temporal features (Hour of Day) resist C4, since the attacker lacks the victim's timing distribution. Device features offer only limited resistance under C3. The system does \textit{not} defend against an adversary who fully replicates the victim's context---same location, device, and timing---which constitutes the residual risk that richer behavioral biometrics (see Limitations) would be needed to address.
