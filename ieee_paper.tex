...
\author{
\IEEEauthorblockN{Satwik Basu (100751), Dr. Sreevani, Dr. Praveen Lalwani, Dr. Pushpinder Singh Patheja}
\IEEEauthorblockA{
\textit{School of Computing Science Engineering and Artificial Intelligence} \\
\textit{VIT Bhopal University, Madhya Pradesh, India} \\
...
\begin{abstract}
Traditional authentication relies on static, binary accept/reject decisions, leaving systems vulnerable to credential theft. This paper presents an Adaptive Authentication System that evaluates contextual features to dynamically assess risk. We deploy a Random Forest classifier achieving 94.45\% accuracy (ROC-AUC 0.9019) with a 27ms median inference latency on a 10,000-event benchmark. Instead of a binary decision, the model drives a three-tier proportional-response framework: ALLOW, MFA (SMS OTP), and BLOCK. Security evaluation across 1,500 adversarial scenarios demonstrates 100\% detection of high-risk attacks with zero false positives. Finally, we bridge the gap between academic research and commercial viability by deploying the system as a production-ready SaaS API platform on AWS with Stripe billing integration. All code and datasets are publicly available.
\end{abstract}
...
\item $\theta_1 \leq r < \theta_2$ (0.7): \textbf{MFA} --- SMS OTP challenge.
...
The system uses four primary tables: \texttt{User} (credentials, MFA secrets), \texttt{LoginLog} (audit trail with risk scores), \texttt{Plan} (subscription tiers), and \texttt{APIKey} (key management with usage tracking).
